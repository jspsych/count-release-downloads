...
async function getPackages() {
  ...
  try {
    ...
    return data.objects
      ...
      .filter(name => name.startsWith('@jspsych/') || name.startsWith('@jspsych-contrib/') || name.startsWith('@jspsych-timelines/'));
  } catch (error) {
...
(async () => {
  ...
  const timelinesCount = results.filter(row => row.package.startsWith('@jspsych-timelines/')).length;
  ...
  for (const row of results) {
    ...
    } else if (row.package.startsWith('@jspsych-timelines/')) {
      cls = 'scope-jspsych-timelines';
    }
